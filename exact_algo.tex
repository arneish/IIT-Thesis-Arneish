...
\textsc{FindAllInstancesApprox} (Algorithm \ref{algo:approx-instances}) to find
every instance of $R$ from \emph{replica($Q$)} that contains $e$. 

Exact \textit{replica} construction (Appendix I) for a
...
\textsc{FindAllInstances} (Appendix I), we identify
...
Algorithm A.1 does not scale well to dense or large
...
Algorithm \ref{algo:approx-instances} describes an approximate method to
implement the idea for accelerated
...
confirmed. Note that child mapping $w$ if inserted into $E_{c}$
